     ...
     var myApp;
      ...
      function loadTableAJAX() {
        ...
        if (<?php echo $_SESSION['entityid']; ?> > 0) {
            ...
            var url = '<?php echo API_HOST_URL; ?>' + '/customer_needs?include=entities&columns=entities.name,id,entityID,qty,rate,rateType,transportationMode,availableDate,expirationDate,originationAddress1,originationCity,originationState,originationZip,originationLat,originationLng,destinationAddress1,destinationCity,destinationState,destinationZip,destinationLat,destinationLng,distance,needsDataPoints,status,contactEmails&filter[]=entityID,eq,' + <?php echo $_SESSION['entityid']; ?> + '&filter[]=expirationDate,ge,' + pastNeedsDateString + '&filter[]=rootCustomerNeedsID,eq,0&satisfy=all&order[0]=createdAt,desc&transform=1';
            var show = false;
        } else {
            var pastNeedsDate = new Date();
            ...
            //var url = '<?php echo API_HOST_URL; ?>' + '/customer_needs?include=entities&columns=entities.name,id,entityID,qty,rate,rateType,transportationMode,availableDate,expirationDate,originationAddress1,originationCity,originationState,originationZip,originationLat,originationLng,destinationAddress1,destinationCity,destinationState,destinationZip,destinationLat,destinationLng,distance,needsDataPoints,status,contactEmails&filter[0]=status,eq,Available&filter[1]=expirationDate,ge,' + pastNeedsDateString + '&filter[2]=rootCustomerNeedsID,eq,0&satisfy=all&order[0]=entityID&order[1]=createdAt,desc&transform=1';
            var url = '<?php echo API_HOST_URL; ?>' + '/customer_needs?include=entities&columns=entities.name,id,entityID,qty,rate,rateType,transportationMode,availableDate,expirationDate,originationAddress1,originationCity,originationState,originationZip,originationLat,originationLng,destinationAddress1,destinationCity,destinationState,destinationZip,destinationLat,destinationLng,distance,needsDataPoints,status,contactEmails&filter[]=expirationDate,ge,' + pastNeedsDateString + '&filter[]=rootCustomerNeedsID,eq,0&satisfy=all&order[0]=entityID&order[1]=createdAt,desc&transform=1';
            var show = true;
        }
            ...
            columns: [
                ...
                { data: "contactEmails", visible: false },
                {
                    ...
                    "mRender": function (o) {
                        ...
                        if (o.status == "Closed") {
                            buttons += " &nbsp;<button class=\"btn btn-primary btn-xs\" role=\"button\"><i class=\"glyphicon glyphicon-ban-circle text\"></i> <span class=\"text\">Closed</span></button>";
                        } else {
    ...
    if(entityID == 0){
      ...
      for (var i = 0; i < dataPoints.object_type_data_points.length; i++) {
          if(dataPoints.object_type_data_points[i].title == "Decals"){
            dpli += '<li>' +
                    'Decals'+
                    '<input id="decals" name="decals" class="form-control mb-sm">\n'+
                    '</li>\n';
          }
          else{
            dpli += '<li>' + dataPoints.object_type_data_points[i].title +
...
<?php } ?>
            ...
            for (var i = 0; i < dataPoints.object_type_data_points.length; i++) {
                ...
                if(dataPoints.object_type_data_points[i].title == "Decals"){
                    dpli += '<li>' +
                            'Decals'+
                            '<input id="decals" name="decals" class="form-control mb-sm" value="'+value+'">\n'+
                            '</li>\n';
                  }
                  else{
